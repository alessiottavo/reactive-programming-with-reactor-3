...
public class Part01FluxTests {
	...
	void expectEmptyFlux() {
		...
		StepVerifier.create(testee.emptyFlux())
		.verifyComplete();
	...
	void expectFooBarFromValues() {
		...
		StepVerifier.create(testee.fooBarFluxFromValues())
		.expectNext("foo", "bar")
		.verifyComplete();
	}
	...
	void expectError() {
		...
		StepVerifier.create(testee.errorFlux())
		.expectError()
		.verify();
	}
	...
	void expectCounter() {
		...
		StepVerifier.create(testee.counter())
