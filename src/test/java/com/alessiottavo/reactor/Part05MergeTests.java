...
public class Part05MergeTests {
	...
	void expectFluxNoOrder() {
		...
		StepVerifier.create(testee.mergeFluxWithInterleave(repositoryWithLatency.findAll(), repository.findAll()))
				.expectSubscription().expectNext(users.get(0)).expectNext(users.get(1)).expectNext(users.get(2))
				.expectNext(users.get(3)).expectNext(users.get(4)).expectNext(users.get(0)).expectNext(users.get(1))
				.expectNext(users.get(2)).expectNext(users.get(3)).verifyComplete();
	}
