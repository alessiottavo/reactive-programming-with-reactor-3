...
public class Part03StepVerifierTests {
	...
	void expectSkylerJesseComplete() {
		...
		StepVerifier.create(testee.skylerJesseComplete()).expectNext(userRepository.findById("swhite").block())
				.expectNext(userRepository.findById("jpinkman").block()).verifyComplete();
	}

	@Test
	void expect3600Elements() {

		StepVerifier.withVirtualTime(() -> testee.flux3600Elements()).expectSubscription()
				.thenAwait(Duration.ofHours(1)).expectNextCount(3600).verifyComplete();
	}
}
